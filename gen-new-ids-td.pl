#!/bin/perl
# Wrapper for emitting all in once for one commune : all "Table décennale", grouped by register
# Like gen-new-ids-nmd-all.pl but inline gen-new-ids-nmd.pl in order to have better output, readier to use

# Differences with gen-new-ids-nmd-all2.pl:
# - IDs in old permalinks were different, hence format_TD() is different from format_3E()
# - no yearly split, so no subhash
...
my (%results, %pretty);
foreach my $type ("Table décennale") {
...
sub process {
	...
	if ($link) {
	    ...
	    # Emit ID for TD:
	    $results{$mainID}{$year} = "    '$mainID' => '$ark',            # $type $commune $id ($desc)\n";
	} else {
...
sub format_TD {
    ...
    # Normalize:
    $id =~ s!/! !g;
    sprintf("%s%s_%04d_%03d_%02d", split(' ', $id)); # Ideally to doble check in old tree!
}
